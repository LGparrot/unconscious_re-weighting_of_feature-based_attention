...
datapath1=wdir+ u'/results/random_samples/'
...
''' this is used to avoid excessive loops to asign values to keys in dic according to indices'''
...
for sub in nSubs:
    ...
    lvls=['corr_1','corr_2','corr_3','corr_4','err_1','err_2','err_3','err_4']
    
    for l in lvls:
        pre_stats['{0}'.format(l)]=[]
    ...
    for block in range(nBlocks):
        ...
        pre_acc=np.zeros(shape=(nTrials))
        pre_al=np.zeros(shape=(nTrials))
        ...
        for t in range(nTrials):#loop through file list
            ...
            if t > 0:
                pre_al[t]=awareness_levels[t-1]
                pre_acc[t]=sub_acc[t-1]
        ...
        pre_targetzip=list(zip(pre_al,pre_acc,switch))
        
        #pre target trials stats
        corr_al1_ind=all_indices((1,1,1),pre_targetzip)
        corr_al2_ind=all_indices((2,1,1),pre_targetzip)
        corr_al3_ind=all_indices((3,1,1),pre_targetzip)
        corr_al4_ind=all_indices((4,1,1),pre_targetzip)
        
        err_al1_ind=all_indices((1,0,1),pre_targetzip)
        err_al2_ind=all_indices((2,0,1),pre_targetzip)
        err_al3_ind=all_indices((3,0,1),pre_targetzip)
        err_al4_ind=all_indices((4,0,1),pre_targetzip)
        
        pre_stats['corr_1'].append(len(corr_al1_ind))
        pre_stats['corr_2'].append(len(corr_al2_ind))
        pre_stats['corr_3'].append(len(corr_al3_ind))
        pre_stats['corr_4'].append(len(corr_al4_ind))
        
        pre_stats['err_1'].append(len(err_al1_ind))
        pre_stats['err_2'].append(len(err_al2_ind))
        pre_stats['err_3'].append(len(err_al3_ind))
        pre_stats['err_4'].append(len(err_al4_ind))
...
# ====================================================================================
    ...
    for key in pre_stats:
        pre_stats[key]=sum(pre_stats[key])
        
    stats=[]
    stats.append(sub)
    for key in pre_stats:
        stats.append(pre_stats[key])
...
#    if sub ==1:
#        with open(datapath1 +'pre_trials_stats_allsubs_mg_beh.csv', 'w') as csvfile:
#            writer = csv.writer(csvfile)
#            [writer.writerow(r) for r in table]
#    
#    else:
#        with open(datapath1 +'pre_trials_stats_allsubs_mg_beh.csv', 'a') as csvfile:
#            writer = csv.writer(csvfile)
#            writer.writerow(stats) 
    
    # with open(datapath1 +'pre_trials_stats_allsubs_mg_beh.csv', 'w') as f:
    #     for key in pre_stats.keys():
    #         f.write("%s,%s\n"%(key,pre_stats[key]))
...
# =============================================================================
    ...
    '''calculate mean RTs, here make random selections from AL2 and AL3 trials - additonal analysis requested from reviewer 2'''
    
        #no switch
    allns=0  
    all_ns1=[]
    all_ns1.append(ns_vertic_rt['1'])
    all_ns1.append(ns_infreq_rt['1'])
    all_ns1.append(ns_freq_rt['1'])
    all_ns1=[item for sublist in all_ns1 for item in sublist] 
    all_ns1=trimm2(all_ns1)
    allns=allns+len(all_ns1)
    
    all_ns2=[]
    all_ns2.append(ns_vertic_rt['2'])
    all_ns2.append(ns_infreq_rt['2'])
    all_ns2.append(ns_freq_rt['2'])
    all_ns2=[item for sublist in all_ns2 for item in sublist] 
    all_ns2=trimm2(all_ns2)
    allns=allns+len(all_ns2)
    
    all_ns3=[]
    all_ns3.append(ns_vertic_rt['3'])
    all_ns3.append(ns_infreq_rt['3'])
    all_ns3.append(ns_freq_rt['3'])
    all_ns3=[item for sublist in all_ns3 for item in sublist] 
    all_ns3=trimm2(all_ns3)
    allns=allns+len(all_ns2)
    
    allsw=0
    w_sw1=[]
    w_sw1.append(freq2vertic_rt['1'])
    w_sw1.append(freq2infreq_rt['1'])
    w_sw1=[item for sublist in w_sw1 for item in sublist] 
    w_sw1=trimm2(w_sw1)
    allsw=allsw+len(w_sw1)
    

    w_sw2=[]
    w_sw2.append(freq2vertic_rt['2'])
    w_sw2.append(freq2infreq_rt['2'])#all sw al2
    w_sw2=[item for sublist in w_sw2 for item in sublist]
    w_sw2=trimm2(w_sw2)
    allsw=allsw+len(w_sw2)
    
    w_sw3=[]
    w_sw3.append(freq2vertic_rt['3'])
    w_sw3.append(freq2infreq_rt['3'])#all sw al3
    w_sw3=[item for sublist in w_sw3 for item in sublist]
    w_sw3=trimm2(w_sw3)
    allsw=allsw+len(w_sw3)
    ...
    #no select randomly as much as you have in sw al1
    if len(w_sw1) < len(w_sw2):

        for t in range(len(w_sw1)):
            random_sw2.append(random.choice(w_sw2))
    
    if len(all_ns1) < len(all_ns2):
        for t in range(len(all_ns1)):
            random_ns2.append(random.choice(all_ns2))
    ...
    if len(w_sw1) < len(w_sw3):

        for t in range(len(w_sw1)):
            random_sw3.append(random.choice(w_sw3))
    if len(all_ns1) < len(all_ns3):
        for t in range(len(all_ns1)):
            random_ns3.append(random.choice(all_ns3))
    ...
    random_sw2=np.nanmean(random_sw2)
    #RT.append(rt_spec_sw_al2)
    RT.append(random_sw2)
    ...
    try:
        random_sw3=np.nanmean(random_sw3)
    except TypeError:
        random_sw3=random_sw3[0]

    RT.append(random_sw3)
    ...
    random_ns2=np.nanmean(random_ns2)

    RT.append(random_ns2)
    avRT.append(ns_al2)
    AL.append('AL2')
    SW.append('repeat')
    SUB.append(sub)
    ...
    try:
        random_ns3=np.nanmean(random_ns3)
    except TypeError:
        random_ns3=random_ns3[0]
    #RT.append(rt_spec_sw_al3)
    RT.append(random_ns3)
    #RT.append(ns_al3)
    avRT.append(ns_al3)
    AL.append('AL3')
    SW.append('repeat')
    ...
    df.to_csv(datapath1 + 'random_mean_RTs_long_allsubs_%s.csv'%(date), sep=',', index= False, header=True)
